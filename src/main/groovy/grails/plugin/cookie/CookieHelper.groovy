...
@Slf4j
class CookieHelper implements ServletAttributes {

    /** 30 days in seconds */
    static final int DEFAULT_COOKIE_AGE = 30 * 24 * 60 * 60
    static final boolean COOKIE_DEFAULT_HTTP_ONLY = true

    static Cookie createCookie(GrailsApplication grailsApplication, HttpServletRequest request, String name, String value, Integer maxAge, String path, String domain, Boolean secure, Boolean httpOnly) {
        Cookie cookie = new Cookie(name, value)
        cookie.path = getDefaultCookiePath(grailsApplication, request, path)
        cookie.maxAge = getDefaultCookieAge(grailsApplication, maxAge)
        // Cookie.setDomain() tries to lowercase domain name and trow NPE if domain is null
        if (domain) {
            cookie.domain = domain
        }
        cookie.secure = getDefaultCookieSecure(grailsApplication, request, secure)
        cookie.httpOnly = getDefaultCookieHttpOnly(grailsApplication, httpOnly)
        cookie.version = 1
        cookie
    }
     ...
     */
    static int getDefaultCookieAge(GrailsApplication grailsApplication, Integer maxAge) {
    ...
    static String getDefaultCookiePath(GrailsApplication grailsApplication, HttpServletRequest request, String path) {
        String cookiePath
        if (path) {
            cookiePath = path
    ...
    static boolean getDefaultCookieSecure(GrailsApplication grailsApplication, HttpServletRequest request, Boolean secure) {
        if (secure != null) {
            return secure
        }
    ...
    static boolean getDefaultCookieHttpOnly(GrailsApplication grailsApplication, Boolean httpOnly) {
        if (httpOnly != null) {
            return httpOnly
        }
    ...
    static void writeCookieToResponse(HttpServletResponse response, Cookie cookie) {
